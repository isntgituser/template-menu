...
/// Map to cache loaded images by name.
extern std::unordered_map<std::string, ID3D11ShaderResourceView *> image_cache;

/// Map to cache loaded fonts by name.
extern std::unordered_map<std::string, ImFont *> font_cache;

namespace textures::image
{
    /**
	* @brief Retrieves an image resource from the cache.
    *
	* @param name The name of the image resource.
	* @return A pointer to the ID3D11ShaderResourceView if found, nullptr otherwise.
	*/
    ID3D11ShaderResourceView *get( const std::string & );

    /**
	* @brief Downloads an image from the web and creates a shader resource view.
	*
	* Downloads image data from a specified URL and creates a DirectX shader resource view.
	*
	* @param device The DirectX device used to create the resource.
	* @param name The name used to cache this image resource.
	* @param url The URL from which to download the image.
	* @return True if the image was successfully created and cached, false otherwise.
	*/
    bool create_from_web( ID3D11Device *, const std::string &, const std::string & );

	/**
	* @brief Loads an image from a local PNG file and creates a shader resource view.
	*
	* @param device The DirectX device used to create the resource.
	* @param name The name used to cache this image resource.
	...
	* @return True if the image was successfully created and cached, false otherwise.
	*/
    bool create_from_png( ID3D11Device *, const std::string &, const std::string & );
}

namespace textures::fonts
{
	/**
	* @brief Retrieves a font from the cache.
	*
	* @param name The name of the font.
	* @return A pointer to the ImFont if found, nullptr otherwise.
	*/
    ImFont *get( const std::string & );

	/**
	* @brief Downloads a TTF font from the web and adds it to ImGui's font atlas.
	*
	* Downloads font data from a specified URL, adds it to the ImGui font atlas, and caches it by name.
	*
	* @param io The ImGuiIO object used for managing fonts.
	* @param name The name used to cache this font.
	* @param url The URL from which to download the font.
	* @param size The size of the font.
	* @param cfg Font configuration settings.
	* @param glyph_ranges The range of glyphs to include.
	* @return True if the font was successfully created and cached, false otherwise.
	*/
    bool create_from_web( const ImGuiIO &, const std::string &, const std::string &, const float, const ImFontConfig * = nullptr, const ImWchar * = nullptr );

	/**
	* @brief Loads a TTF font from a local file and adds it to ImGui's font atlas.
	*
	* @param io The ImGuiIO object used for managing fonts.
	* @param name The name used to cache this font.
	* @param path The file path to the TTF font.
	* @param size The size of the font.
	* @param cfg Font configuration settings.
	* @param glyph_ranges The range of glyphs to include.
	* @return True if the font was successfully created and cached, false otherwise.
	*/
    bool create_from_ttf( const ImGuiIO &, const std::string &, const std::string &, const float, const ImFontConfig * = nullptr, const ImWchar * = nullptr );
}